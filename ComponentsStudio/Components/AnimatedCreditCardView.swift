...
/// and an upper-edge adaptation of the supplied projectile particle effect.
...
private struct CreditCardParticles: View {
    ...
    private func particle(index: Int) -> some View {
        ...
        let origin = CreditCardParticleEngine.origin(
            for: index,
            size: size
        )
        ...
        let speed = CreditCardParticleEngine.speed(
            for: index,
            maximum: specs.particleSpeed
        )
        let diameter = CreditCardParticleEngine.diameter(
            for: index,
            maximum: specs.particleSize
        )
        let opacity = CreditCardParticleEngine.opacity(
            for: index,
            maximum: specs.particleOpacity
        )
        let colorIndex = Int(
