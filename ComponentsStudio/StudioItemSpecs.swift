...
struct MovingGradientCardSpecs: Equatable {
    var detail: Double
    ...
    var material: Double
    var gradientMethod: Double
    var gradientBalance: Double
    var stop0R: Double
    var stop0G: Double
    var stop0B: Double
    var stop1R: Double
    ...
    @MainActor
    init(prefix: String, _ state: ComponentSpecState, sheet: ComponentSpecSheet) {
        func v(_ key: String) -> Double { sheet.value("\(prefix)\(key)", in: state) }
        detail = v("Detail")
        intensity = v("Intensity")
        twist = v("Twist")
        warp = v("Warp")
        zoom = v("Zoom")
        rotationSpeed = v("Speed")
        morphSpeed = v("Morph")
        material = v("Material")
        gradientMethod = v("Method")
        gradientBalance = v("Balance")
        stop0R = v("0R"); stop0G = v("0G"); stop0B = v("0B")
        stop1R = v("1R"); stop1G = v("1G"); stop1B = v("1B")
        stop2R = v("2R"); stop2G = v("2G"); stop2B = v("2B")
    }
}

struct AnimatedCreditCardSpecs: Equatable {
    var debit: MovingGradientCardSpecs
    var checking: MovingGradientCardSpecs
    var snakePeriod: Double
    ...
    var dustImpulse: Double
    var dustSpeed: Double
    var flipResponse: Double
    var flipDamping: Double
    var flipPerspective: Double
    var flipLift: Double

    @MainActor
    init(_ state: ComponentSpecState, sheet: ComponentSpecSheet) {
        debit = MovingGradientCardSpecs(prefix: "d", state, sheet: sheet)
        checking = MovingGradientCardSpecs(prefix: "c", state, sheet: sheet)
        snakePeriod = sheet.value("snakePeriod", in: state)
        ...
        particleSpread = sheet.value("particleSpread", in: state)
        particleSize = sheet.value("particleSize", in: state)
        particleOpacity = sheet.value("particleOpacity", in: state)
        dustImpulse = sheet.value("dustImpulse", in: state)
        dustSpeed = sheet.value("dustSpeed", in: state)
        flipResponse = sheet.value("flipResponse", in: state)
        flipDamping = sheet.value("flipDamping", in: state)
        flipPerspective = sheet.value("flipPerspective", in: state)
        flipLift = sheet.value("flipLift", in: state)
    }
}
...
extension StudioItem {
    ...
    func swiftCodeSnippet(state: ComponentSpecState, presetLabel: String) -> String {
        ...
        case .animatedCreditCard:
            ...
            return """
            // Generated by Component Studio · preset: \(presetLabel)
            // Flex Cards — Figma carousel + Moving gradient shader

            // Debit shader
            let debitDetail: Float = \(fmt(s.debit.detail))
            let debitIntensity: Float = \(fmt(s.debit.intensity))
            let debitZoom: Double = \(fmt(s.debit.zoom))
            let debitMorph: Float = \(fmt(s.debit.morphSpeed))

            // Checking shader
            let checkingDetail: Float = \(fmt(s.checking.detail))
            let checkingIntensity: Float = \(fmt(s.checking.intensity))
            let checkingZoom: Double = \(fmt(s.checking.zoom))
            let checkingMorph: Float = \(fmt(s.checking.morphSpeed))

            let snakePeriod: Double = \(fmt(s.snakePeriod))
            let snakeWidth: CGFloat = \(fmt(s.snakeWidth))
            let particleCount: Int = \(s.particleCount)
            let dustImpulse: Double = \(fmt(s.dustImpulse))
            let flip: Animation = .spring(response: \(fmt(s.flipResponse)), dampingFraction: \(fmt(s.flipDamping)))
            """
    ...
    private static func shaderControls(prefix: String) -> [ComponentSpecControl] {
        [
            .init(id: "\(prefix)Detail", label: "Detail", kind: .slider(0.0...5.0, format: "%.2f")),
            .init(id: "\(prefix)Intensity", label: "Intensity", kind: .slider(0.0...5.0, format: "%.2f")),
            .init(id: "\(prefix)Twist", label: "Twist", kind: .slider(0.0...7.0, format: "%.2f")),
            .init(id: "\(prefix)Warp", label: "Flow", kind: .slider(0.0...1.5, format: "%.2f")),
            .init(id: "\(prefix)Zoom", label: "Zoom", kind: .slider(0.0...100.0, format: "%.0f%%")),
            .init(id: "\(prefix)Speed", label: "Speed", kind: .slider(0.0...100.0, format: "%.0f%%")),
            .init(id: "\(prefix)Morph", label: "Morph", kind: .slider(0.0...5.0, format: "%.2f")),
            .init(id: "\(prefix)Material", label: "Material", kind: .slider(0.0...4.0, format: "%.0f")),
            .init(id: "\(prefix)Method", label: "Method", kind: .slider(0.0...2.0, format: "%.0f")),
            .init(id: "\(prefix)Balance", label: "Balance", kind: .slider(-100.0...100.0, format: "%.0f%%")),
            .init(id: "\(prefix)0", label: "Color 1", kind: .color),
            .init(id: "\(prefix)1", label: "Color 2", kind: .color),
            .init(id: "\(prefix)2", label: "Color 3", kind: .color),
        ]
    }

    /// Figma Moving gradient defaults, with per-card silk palettes sampled
    /// from the card_carrousel file (debit = deeper navy, checking = washed silk).
    private static func shaderDefaults(
        prefix: String,
        zoom: Double,
        balance: Double,
        stops: [[Double]]
        ...
        [
            ...
            "\(prefix)0R": stops[0][0], "\(prefix)0G": stops[0][1], "\(prefix)0B": stops[0][2],
            "\(prefix)1R": stops[1][0], "\(prefix)1G": stops[1][1], "\(prefix)1B": stops[1][2],
            "\(prefix)2R": stops[2][0], "\(prefix)2G": stops[2][1], "\(prefix)2B": stops[2][2],
        ]
    }
    ...
    private static let animatedCreditCardSheet = ComponentSpecSheet(
        categories: [
            ComponentSpecCategory(
                id: "debit",
                label: "Debit",
                controls: shaderControls(prefix: "d")
            ),
            ComponentSpecCategory(
                id: "checking",
                label: "Checking",
                controls: shaderControls(prefix: "c")
            ),
            ComponentSpecCategory(
                id: "stroke",
                label: "Stroke",
                controls: [
                    .init(id: "snakePeriod", label: "Cycle", kind: .slider(1.0...8.0, format: "%.1fs")),
                    .init(id: "snakeWidth", label: "Stroke", kind: .slider(0.4...2.0, format: "%.1f")),
                    .init(id: "auraBlur", label: "Aura blur", kind: .slider(0.0...24.0, format: "%.0f")),
                    .init(id: "auraOpacity", label: "Aura", kind: .slider(0.0...0.40, format: "%.2f")),
                ]
            ),
            ComponentSpecCategory(
                id: "dust",
                label: "Dust",
                controls: [
                    .init(id: "particleCount", label: "Count", kind: .slider(4.0...60.0, format: "%.0f")),
                    .init(id: "particleDuration", label: "Life", kind: .slider(0.5...4.0, format: "%.1fs")),
                    .init(id: "particleSpeed", label: "Speed", kind: .slider(20.0...160.0, format: "%.0f")),
                    .init(id: "particleGravity", label: "Gravity", kind: .slider(0.0...300.0, format: "%.0f")),
                    .init(id: "particleSpread", label: "Spread", kind: .slider(0.02...0.45, format: "%.2f")),
                    .init(id: "particleSize", label: "Size", kind: .slider(0.8...6.0, format: "%.1f")),
                    .init(id: "particleOpacity", label: "Opacity", kind: .slider(0.08...1.0, format: "%.2f")),
                    .init(id: "dustImpulse", label: "Burst", kind: .slider(0.0...3.0, format: "%.2f")),
                    .init(id: "dustSpeed", label: "Lift", kind: .slider(0.0...180.0, format: "%.0f")),
                ]
            ),
            ComponentSpecCategory(
                id: "flip",
                label: "Flip",
                controls: [
                    .init(id: "flipResponse", label: "Response", kind: .slider(0.20...1.00, format: "%.2f")),
                    .init(id: "flipDamping", label: "Damping", kind: .slider(0.40...1.00, format: "%.2f")),
                    .init(id: "flipPerspective", label: "Depth", kind: .slider(0.20...0.90, format: "%.2f")),
                    .init(id: "flipLift", label: "Height", kind: .slider(0.0...36.0, format: "%.0f")),
                ]
            ),
        ],
        defaults: {
            var values: [String: Double] = [
                ...
                "flipResponse": 0.55,
                "flipDamping": 0.72,
                "flipPerspective": 0.55,
                "flipLift": 18,
            ]
            // Debit — deeper navy silk from the Figma still.
            values.merge(shaderDefaults(
                prefix: "d",
                zoom: 72,
                balance: -8,
                stops: [
            ...
            // Checking — lighter washed silk, same algorithm, isolated knobs.
            values.merge(shaderDefaults(
                prefix: "c",
                zoom: 78,
                balance: 16,
