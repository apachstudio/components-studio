...
extension StudioItem {
    ...
    func swiftCodeSnippet(state: ComponentSpecState, presetLabel: String) -> String {
        ...
        case .animatedCreditCard:
            ...
            return """
            // Generated by Component Studio · preset: \(presetLabel)
            // AnimatedCreditCardView — AI snake + projectile particles
            ...
            let particleMaximumSpeed: Double = \(fmt(s.particleSpeed))
            let particleGravity: Double = \(fmt(s.particleGravity))
            let particleSpread: Double = \(fmt(s.particleSpread)) * .pi
            let particleMaximumSize: CGFloat = \(fmt(s.particleSize))
            let particleMaximumOpacity: Double = \(fmt(s.particleOpacity))
            """
    ...
    private static let animatedCreditCardSheet = ComponentSpecSheet(
        ...
        defaults: [
            // Snake values match the AI Search pill's exact rest-state tuning.
            "snakePeriod": 4.2,
            "snakeWidth": 0.9,
            "auraBlur": 10.0,
            "auraOpacity": 0.13,
            // Motion preserves the supplied ParticleEffectView.swift values.
            "particleCount": 24.0,
            "particleDuration": 2.0,
            "particleSpeed": 80.0,
            "particleGravity": 147.15,
            // Sparse upper-edge distribution matched to the button reference.
            "particleSpread": 0.18,
            "particleSize": 2.2,
